...
return new class extends Migration
{
        ...
        ProductChannelMapping::query()
            ...
            ->chunkById(100, function ($mappings) use ($repair, &$visited): void {
                foreach ($mappings as $mapping) {
                    ...
                    $isCrossLocalizedSize = preg_match(
                        '/^WooCommerce [A-Z]{2} #\d+: WordPress nie powiązał tłumaczeń wartości globalnego atrybutu: Wartość atrybutu \d+ ma już język [a-z-]+ zamiast oczekiwanego [a-z-]+\.$/u',
                        $reason,
                    ) === 1;

                    if (! $product instanceof Product
                        || isset($visited[$product->id])
                        || ! $isCrossLocalizedSize
                    ) {
                        continue;
                    }
    ...
    {
        // Deliberate no-op: the failed attempt rolled back its remote rename.
    }
};
