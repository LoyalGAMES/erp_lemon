...
return new class extends Migration
{
        ...
        ProductChannelMapping::query()
            ...
            ->chunkById(100, function ($mappings) use ($repair, &$visited): void {
                foreach ($mappings as $mapping) {
                    ...
                    $isMisnamedTerm = preg_match(
                        '/^WooCommerce [A-Z]{2} #\d+: Istniejące polskie tłumaczenie #\d+ \([^,]+, [^)]+\) wartości #\d+ nie odpowiada opcji .+\.$/u',
                        $reason,
                    ) === 1;
    ...
    {
        // Deliberate no-op: remote mutations occur only in the guarded repair.
    }
};
