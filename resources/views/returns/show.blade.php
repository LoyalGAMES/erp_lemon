...
@php
    ...
    foreach ($returnCase->customerMessages as $message) {
        $pushEvent($message->sent_at ?? $message->created_at, 'Wiadomość do klienta', $message->subject);
    }
...
@push('styles')
    <style>
        ...
        .return-summary { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 12px; margin-bottom: 16px; }
        .return-summary-card { padding: 14px 16px; }
        .return-summary-card span { display: block; color: var(--muted); font-size: 12px; font-weight: 720; margin-bottom: 4px; }
        .return-summary-card strong { display: block; font-size: 20px; line-height: 1.15; }
        .return-layout { display: grid; grid-template-columns: minmax(0, 1.45fr) minmax(320px, .75fr); gap: 16px; align-items: start; }
        .return-stack { display: grid; gap: 16px; }
        .return-section-body { padding: 16px; }
        .return-info-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; }
        ...
        .return-service-actions > span { width: fit-content; }
        ...
        .return-service-actions .return-message-panel { min-width: 0; margin-top: 0; white-space: normal; border-top: 1px solid var(--border); padding-top: 10px; }
        .return-service-actions .return-message-panel summary { cursor: pointer; color: var(--brand-dark); font-weight: 760; }
...
@section('content')
    ...
    <section class="return-layout">
        <div class="return-stack">
            ...
            <section class="return-info-grid">
                <article class="card">
                    <div class="panel-header">
                ...
                <div class="return-section-body return-record-list">
                    @forelse ($returnCase->customerMessages->take(8) as $message)
                        <article class="return-record-card">
                            ...
                            <div class="return-record-preview">{{ \Illuminate\Support\Str::limit($message->body, 220) }}</div>
                        </article>
                    @empty
